...
def extract_features(coords):
    ...
    features["face_ratio"]  = round(face_width / face_height, 3)  # 越接近1越圓
    ...
    features["upper_zone_ratio"]  = round(upper_zone  / total_zone, 3)
    features["middle_zone_ratio"] = round(middle_zone / total_zone, 3)
    features["lower_zone_ratio"]  = round(lower_zone  / total_zone, 3)
    ...
    features["left_eye_ratio"]  = round(left_eye_h  / left_eye_w,  3)  # 開眼度
    features["right_eye_ratio"] = round(right_eye_h / right_eye_w, 3)
    ...
    features["eye_gap_ratio"] = round(eye_gap / features["eye_width_avg"], 3)
    # 理想值約 1.0 (兩眼相距一個眼寬)
    ...
    features["brow_eye_ratio"] = round(
        ((left_brow_w + right_brow_w) / 2) / features["eye_width_avg"], 3
    )  # 眉長 vs 眼長，>1.0 代表眉毛較長

    # 眉眼距離
    brow_eye_dist = dist(coords["left_brow_top"], coords["left_eye_top"])
    features["brow_eye_distance"] = round(brow_eye_dist / face_height, 3)

    # ── 鼻子特徵 ──
    nose_w = dist(coords["nose_left"], coords["nose_right"])
    nose_h = dist(coords["nose_bridge_top"], coords["nose_tip"])
    features["nose_width_ratio"]  = round(nose_w / face_width, 3)  # 鼻寬 vs 臉寬
    features["nose_height_ratio"] = round(nose_h / face_height, 3)
    features["nose_shape_ratio"]  = round(nose_w / nose_h, 3)  # 越大越寬扁，越小越長挺

    # 鼻孔外露度 (nostril_exposure_ratio)
    nose_wings_y = (coords["nose_left"][1] + coords["nose_right"][1]) / 2.0
    features["nostril_exposure_ratio"] = round((nose_wings_y - coords["nose_tip"][1]) / face_height, 3)

    # ── 嘴巴特徵 ──
    mouth_w = dist(coords["mouth_left"],  coords["mouth_right"])
    mouth_h = dist(coords["mouth_top"],   coords["mouth_bottom"])
    features["mouth_width_ratio"]  = round(mouth_w / face_width, 3)
    features["mouth_shape_ratio"]  = round(mouth_w / mouth_h, 3)  # 嘴形寬度比
    features["lip_thickness"]      = round(mouth_h / face_height, 3)
    ...
    features["mouth_corner_upward_ratio"] = round((mouth_center_y - mouth_corners_y) / max(1.0, mouth_w), 3)

    # ── 下巴特徵 ──
    jaw_w = dist(coords["left_cheek"], coords["right_cheek"])
    features["jaw_ratio"] = round(jaw_w / face_width, 3)  # 下巴相對臉寬
    ...
    if "left_iris_center" in coords and "right_iris_center" in coords:
        ...
        left_pos = (coords["left_eye_bottom"][1] - coords["left_iris_center"][1]) / left_eye_height
        right_pos = (coords["right_eye_bottom"][1] - coords["right_iris_center"][1]) / right_eye_height
    ...
    features["glabella_width_ratio"] = round(glabella_w / face_width, 3)
    ...
    features["philtrum_ratio"] = round(philtrum_len / face_height, 3)
