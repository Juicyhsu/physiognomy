...
def draw_annotations(image, coords, features):
    ...
    # 計算解析度適應縮放因子 (以寬度 640 作為基準)
    scale = max(1.0, w / 640.0)
    r = max(2, int(4 * scale))
    thickness = max(1, int(1 * scale))
    thickness_bold = max(1, int(2 * scale))
    ...
    def draw_point(name, color, radius=r):
        if name in coords:
            cv2.circle(img, coords[name], radius, color, -1)

    def draw_line(p1_name, p2_name, color, line_thickness=thickness):
        if p1_name in coords and p2_name in coords:
            cv2.line(img, coords[p1_name], coords[p2_name], color, line_thickness)
    ...
    for side in ["left", "right"]:
        ...
        draw_line(f"{side}_brow_inner", f"{side}_brow_outer", colors["brow"], thickness_bold)
    ...
    for pt_name, label in [
        ...
        if pt_name in coords:
            y = coords[pt_name][1]
            cv2.line(img, (0, y), (w, y), (255, 255, 100), thickness, cv2.LINE_AA)
            cv2.putText(img, label, (int(5 * scale), y - int(4 * scale)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35 * scale, (255, 255, 100), font_thickness)
    ...
    rect_top = int(10 * scale)
    rect_bottom = rect_top + int(5 * scale) + len(info_lines) * line_height
    rect_left = panel_x - int(5 * scale)
    rect_right = w - int(5 * scale)
    
    cv2.rectangle(img, (rect_left, rect_top), (rect_right, rect_bottom),
                  (30, 30, 30), -1)
    
    for i, line in enumerate(info_lines):
        text_y = rect_top + int(14 * scale) + i * line_height
        cv2.putText(img, line, (panel_x, text_y),
